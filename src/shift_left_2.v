// ============================================================
// shift_left2.v
// Fixed shift-left-by-2 (multiply by 4, word alignment).
// Implemented as bit concatenation, not real ALU shift logic --
// kept off the ALU's critical path per architecture decision.
//
...
module shift_left2 #(
    ...
    // Take the lowest OUT_WIDTH bits of the shifted value:
    //  - if OUT_WIDTH == IN_WIDTH+2 (jump case), this is an exact fit, no loss
    //  - if OUT_WIDTH == IN_WIDTH (branch case), this drops the top 2 bits,
    //    which are redundant sign-extension copies, so nothing meaningful is lost
